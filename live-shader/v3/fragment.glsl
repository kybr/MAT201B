...
uniform vec2 Focus = vec2(-0.51, 0.54);
...
vec3 color(int iteration, float sqLengthZ) {
  ...
  // just color it black
  if (iteration == MaxIterations) return vec3(0.0);

  // Else we give it a smoothed color
  float ratio =
      (float(iteration) - log2(log2(sqLengthZ))) / float(MaxIterations);
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  vec2 c = (-1.0 + 2.0 * fragCoord / iResolution.xy) *
